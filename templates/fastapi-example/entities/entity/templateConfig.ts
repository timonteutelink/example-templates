...
const templateSettingsSchema = z.object({
  entity_name: z
    ...
    .optional()
    .default("post")
    .describe("The name of the FastAPI application, used in the Dockerfile"),
});
  ...
  sideEffects: [
    {
      ...
      apply: async (settings, fileContents) => {
        ...
        if (
          fileContents.includes(importStatement) ||
          fileContents.includes(includeRouterStatement)
        ) {
          return null;
        }

        let lines = fileContents.split("\n");
        let insertImportBefore = lines.findIndex((line) =>
          line.startsWith("###IMPORT_ROUTER_PLACEHOLDER###"),
        );
        let insertRouterBefore = lines.findIndex((line) =>
          line.startsWith("###REGISTER_ROUTER_PLACEHOLDER###"),
        );
